...
def _extract_diagnosis_condition(text: str, is_hindi: bool = False) -> str:
    ...
    if any(k in text_lower for k in ["chest pain", "crushing", "angina"]):
        return "तीव्र सीने में दर्द / एंजाइना लक्षण (अस्पताल ईसीजी आवश्यक)" if is_hindi else "Acute Cardiac Discomfort / Angina Symptoms (Requires Immediate Hospital ECG)"
    elif any(k in text_lower for k in ["gala", "throat", "sore throat", "pharyngitis"]):
        return "गले का संक्रमण / ग्रसनीशोथ (Pharyngitis)" if is_hindi else "Acute Pharyngitis / Upper Respiratory Tract Symptoms"
    elif any(k in text_lower for k in ["fever", "bukhar", "high temperature", "chills"]):
        return "तीव्र ज्वर / वायरल बुखार (Viral Fever)" if is_hindi else "Acute Febrile Presentation / Viral Infection"
    elif any(k in text_lower for k in ["headache", "sar dard", "migraine"]):
        return "सिरदर्द / माइग्रेन के लक्षण" if is_hindi else "Acute Cephalea / Tension-Type Headache"
...
async def classify_medical_image_type(image_base64: Optional[str], caption: Optional[str] = None) -> str:
    """
    Uses Gemini / OpenRouter Vision to accurately classify incoming WhatsApp media into:
    ...
    Falls back gracefully to caption heuristics or prescription default.
    """
...
def format_compact_whatsapp_card(text: str, lang: str = "en") -> str:
    ...
    conf_match = (
        re.search(r'\((\d+%\s*(?:Confidence|Consensus)?)\)', text, re.IGNORECASE) or
    ...
    if conf_match:
        ...
        cons_label = f"📊 काउंसिल सहमति: {conf_str} (चिकित्सक समीक्षा अनुशंसित)" if is_hindi else f"📊 Council Consensus: {conf_str} (Physician review recommended)"
        lines.append(cons_label)
    else:
        lines.append("⚖️ नैदानिक मूल्यांकन: मल्टी-एजेंट सुरक्षा समीक्षा संपन्न" if is_hindi else "⚖️ Clinical Assessment: Multi-agent safety review (Physician verification required)")
