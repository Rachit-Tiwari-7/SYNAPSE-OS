...
"""
    ...
    assert "🔴 SYNAPSE EMERGENCY TRIAGE — CRITICAL" in card
...
"""
    ...
    assert "*" not in card
    assert "_" not in card
    assert "🟢 SYNAPSE HOME CARE & MONITORING" in card
...
def test_compact_whatsapp_card_from_plain_unformatted_input():
    ...
    """
    plain_input = """SYNAPSE EMERGENCY CLINICAL ASSESSMENT
Patient Status: EMERGENCY CARE (IMMEDIATE)
...
"""
    ...
    assert "SYNAPSE HOME CARE & MONITORING" not in output
    assert "SYNAPSE CLINICAL ASSESSMENT" not in output
