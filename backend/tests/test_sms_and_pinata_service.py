...
async def test_sms_symptom_triage_with_pinata_ipfs():
    """Tests that symptom triage generates AI advice."""
...
async def test_sms_outbreak_and_vaccination_routes():
    """Tests option 8 and option 7 over SMS."""
    res_outbreak = await process_sms_inbound_webhook(from_number="+919876543210", body="8 Delhi")
    assert res_outbreak["type"] == "outbreak_alert"

    res_vax = await process_sms_inbound_webhook(from_number="+919876543210", body="7")
    assert res_vax["type"] == "vaccination_schedule"
    assert "VACCINE" in res_vax["reply"]
...
async def test_api_twilio_webhook_endpoint():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert "RED ALERT" in resp.text or "EMERGENCY" in resp.text
