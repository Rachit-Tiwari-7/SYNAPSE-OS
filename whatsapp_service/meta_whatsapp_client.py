"""
whatsapp_service — meta_whatsapp_client.py
Official Meta WhatsApp Cloud API (Graph API v20.0) Client.
Supports outbound text, interactive button/list menus, image analysis, media downloads, and read receipts.
Includes full sandbox simulation mode for testing when credentials are unconfigured.
"""
...
def _clean_recipient_phone(to_phone: str) -> str:
    """Sanitizes recipient phone number (removes '@c.us', '@s.whatsapp.net', spaces, hyphens, and leading '+')."""
...
async def send_whatsapp_message(to_phone: str, text: str) -> Dict[str, Any]:
    ...
    if not settings.WHATSAPP_CLOUD_API_TOKEN or not settings.WHATSAPP_PHONE_NUMBER_ID:
        ...
        return {
            ...
            "info": "Meta WhatsApp API token or Phone ID not set. Output captured in local audit stream.",
            "recipient": clean_to,
            "preview": text[:120] + "..." if len(text) > 120 else text,
            "full_text": text
        }

    # Meta message text limit is 4096 characters, chunk at 3800 for safety
    max_chunk = 3800
    ...
    formatted_buttons = [
        {
            ...
            "reply": {
                ...
                "title": btn.get("title", f"Option {i}")[:20]
            }
        }
...
async def mark_message_as_read(message_id: str) -> bool:
    """
    Marks an incoming WhatsApp message as 'read' in real-time (blue ticks).
    """
